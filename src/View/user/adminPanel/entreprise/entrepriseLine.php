<div class="container line <?= $entreprise->getEstValide() ? "greenHover" : "yellowHover" ?>">
    ...
    <label class="lineTelephoneEntreprise"><a class="link"
                                              href="tel:<?= $entreprise->getTelephoneEntreprise() ?>"><?= htmlspecialchars($entreprise->getTelephoneEntreprise()) ?></a></label>
    <label class="lineSiteWebEntreprise"><a class="link"
                                            href="https://<?= rawurlencode($entreprise->getSiteWebEntreprise()) ?>">Site
            web</a></label>
    <a class="button consulterButton"
       href="frontController.php?action=panelGestionEntreprise&controller=PanelAdmin&siret=<?= rawurlencode($entreprise->getSiret()) ?>">Consulter</a>
</div>